...
async def consultar_plugins():
    try:
        return await _controller.listar_plugins()
    except:
        pass
...
async def upload_plugin(hook_file: UploadFile = File(...), plugin_file: UploadFile = File(...)):
    try:
        ...
        elif not hook_filename.endswith('__hooks.py'):
            raise HTTPException(status_code=400, detail="Nome do arquivo do hooks invalido")

        
        resultado = await _controller.importar_plugins(hook_file, plugin_file)
